...
mpirun --hostfile machinefile.$JOB_ID -np 1 snappyHexMesh -parallel -overwrite > logs/04_snappyHexMesh.log 2>&1
...
mpirun --hostfile machinefile.$JOB_ID -np 1 potentialFoam -parallel > logs/11_potentialFoam.log 2>&1
mpirun --hostfile machinefile.$JOB_ID -np 1 simpleFoam -parallel > logs/12_simpleFoam.log 2>&1
